...
Open the Vite dev URL, paste a video URL, leave the default target
dialogue (`My mind rebels at stagnation`), and submit. By default the
pipeline finds that line's **first appearance** and stops. Check
**Show all frames with dialogues** if you want every distinct on-screen
line instead.

## Full-stack API

```
POST /videos/process            {url, target_text?, force?, scan_all?} -> {job_id, video_id, status}
...
`scan_all` defaults to `false`: the worker runs
`core.pipeline.process_video_full(..., scan_all=False)`, which finds the
first dialogue (or the given target) via early-stop coarse scan +
backward refine, then stops. Set `scan_all=true` to also walk the whole
video with `core/dialogue_scan.py` for the "display ALL detected
dialogues" feature.
