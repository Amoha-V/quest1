...
| Short-lived on-screen text (shorter than the coarse interval) | `change_detector.py` - a fine-interval (0.15s), OCR-free pixel-diff pre-pass that flags candidate timestamps for the coarse OCR loop to also check | Solved |
...
- **Not implemented**: `scan_all` (`core/dialogue_scan.py::scan_all_dialogues()`,
  the frontend's "Scan all dialogues" toggle) only lists every on-screen
  (OCR) dialogue line. There's no equivalent listing of every *spoken*
  dialogue line with its frame - the ASR side is only ever used for a
  single target-text lookup (`core/combined_resolver.py`), never for an
  exhaustive "every spoken line" scan. The pieces to build it already
  exist (the cached ASR transcript has every segment's timestamp, and
  `extract_frame()` can pull a frame at any timestamp), so this is a scope
